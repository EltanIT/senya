...
class IndoorNavigation(
    ...
    companion object {
        @JvmStatic var redactPositionState = false
    }
...
//        }
    ...
    fun checkIndoorPoints(position: Point) {
        ...
        if (
            ...
            CoroutineScope(Dispatchers.Main).launch {
                for (i in 0..<newPoints.size){
                    ...
                    indoorRoutePolylineObject = map.mapWindow.map.mapObjects.addPolyline(Polyline(newPoints))
        ...
        if (!isIndoor){
            ...
            indoorRoutePolylineObject = map.mapWindow.map.mapObjects.addPolyline(Polyline(indoorRoute))
        }
